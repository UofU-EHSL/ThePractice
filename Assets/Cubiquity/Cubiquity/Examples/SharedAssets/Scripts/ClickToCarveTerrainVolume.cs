...
public class ClickToCarveTerrainVolume : MonoBehaviour
{
	...
	void Update ()
	{
        ...
        //Debug.Log("ray" + pickResult.volumeSpacePos);
        pickResult.volumeSpacePos = new Vector3(Convert.ToInt32(tool.transform.position.x), Convert.ToInt32(tool.transform.position.y), Convert.ToInt32(tool.transform.position.z));
        Debug.Log("hand" + pickResult.volumeSpacePos);     
        ...
        if (!isMouseAlreadyDown)
        {
            ...
            PickSurfaceResult pickResult;
            PickSurfaceResult pickResult2;
            bool hit = Picking.PickSurface(terrainVolume, ray, 1000.0f, out pickResult);

            // If we hit a solid voxel then create an explosion at this point.
            pickResult2.volumeSpacePos = new Vector3(Convert.ToInt32(tool.transform.position.x), Convert.ToInt32(tool.transform.position.y), Convert.ToInt32(tool.transform.position.z));
            //if (hit)
            //{
            Debug.Log(pickResult2.volumeSpacePos);
            Debug.Log(pickResult.volumeSpacePos);
            DestroyVoxels((int)pickResult2.volumeSpacePos.x, (int)pickResult2.volumeSpacePos.y, (int)pickResult2.volumeSpacePos.z, range);
            //}
